...
answer:Functional components are basic javascript functions that mostly render UI, accept props as arguments, return React elements and are considered stateless. Whereas, Class components are considered stateful and deal with logic and state, extends the React.Component and utilize React Lifecycle methods.

2. When does a componentWillMount function called? What about a componentDidUpdate?

answer: During the React lifecycle of 1) Birth/Mounting, 2) Growth/Updating, and 3) Death/Unmounting you call the componentWillMount during the Mounting phase and set whatever initial data you want access to, defined on the constructor of this phase. During the growth/updating phase is when React reacts to changes in state data and the componentDidUpdate can be called.

3. Define stateful logic.

answer: "Stateful logic is logic that is built into a component. For example, it can be a function that handles a click event, a function that sets a toggle state, or a function that formats data before it gets displayed." -module project definition.

4. What are the three step of creating a successful test? What is done in each phase?

answer: 1. Arrange: call the render method and render React to virtual DOM. 2. Act: query to find a node we expect to see rendered to the DOM. 3. Assert: confirm the node is behaving as we expect.
